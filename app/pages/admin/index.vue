...
const fetchInspectors = async () => {
  try {
    const res = await $fetch<Inspector[]>("/api/admin/inspectors", {
      headers: getAuthHeaders(),
    });
...
const addInspector = async () => {
  if (!newInspector.value.name || !newInspector.value.token) {
    showToast("请填写姓名和令牌");
    return;
  }
  try {
    await $fetch("/api/admin/inspectors/add", {
      method: "POST",
      headers: getAuthHeaders(),
      body: newInspector.value,
    });
    showSuccessToast("添加成功");
    newInspector.value = { name: "", token: "" };
    showAddInspectorModal.value = false;
    fetchInspectors();
  } catch (e: any) {
    showToast(e.data?.statusMessage || "添加失败");
  }
};
...
<template>
  <div class="admin-page">
    ...
    <template v-else>
      ...
      <van-popup
        v-model:show="showInspectorsModal"
        position="bottom"
        round
        :style="{ height: '70%', background: '#f7f8fa' }"
      >
        <div class="modal-content">
          <div class="modal-header">
      ...
      >
        <van-cell-group inset style="margin: 16px 0">
          <van-field
            ...
            label="姓名"
            placeholder="请输入检票员姓名"
            required
          />
          <van-field
            v-model="newInspector.token"
            label="令牌"
            placeholder="请输入登录令牌"
            required
          />
