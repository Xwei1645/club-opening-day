...
export default defineEventHandler(async (event) => {
  ...
  if (!raw) {
    throw createError({ statusCode: 400, statusMessage: 'fingerprintHash is required.' })
  }
  ...
  if (cfg.drawStatus !== 'DONE') {
    return { stage: 'waiting', drawAt: cfg.drawAt, publishStatus: cfg.publishStatus }
  }

  if (cfg.publishStatus === 'HIDDEN') {
    return { stage: 'hidden', drawAt: cfg.drawAt, resultGeneratedAt: cfg.resultGeneratedAt }
  }
